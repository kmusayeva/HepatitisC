...
class DataVisualize:
    ...
    def GenderDisease(self):
        ...
        gender_diagnosis = (
            self.df.groupby(["Sex", "Diagnosis_Multi"])
            .size()
            .groupby(level=0)
        ...
        for i, v in enumerate(gender_counts.values):
            ax1.text(
                i,
                v + max(gender_counts.values) * 0.01,
                str(v),
                ha="center",
                fontsize=11,
            )
        ...
        gender_diagnosis.unstack().plot(
            kind="bar", ax=ax2, stacked=True, rot=0, color=self.colors_list
        )
        ax2.set_title("Disease Prevalence per Gender (Percent)", fontsize=14)
        ax2.set_ylabel("Percentage", fontsize=12)
        ax2.set_xlabel("Gender", fontsize=12)
        ax2.legend(
            title="Diagnosis", bbox_to_anchor=(1.05, 1), loc="upper left", fontsize=10
        )
    ...
    def missingness(self):
        ...
        df_missingness = (
            self.df.drop(columns=["Diagnosis", "Diagnosis_Binary"])
    ...
    def BiomarkerSummary(self, var):
        ...
        return (
            bio_df.groupby("Diagnosis_Multi")
            .describe()
            .sort_values(
                by=["Diagnosis_Multi"],
                key=lambda x: x.map(self.diagnosis_dict),
